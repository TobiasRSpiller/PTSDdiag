...
1.  **Re-experiencing in the present**: \\\ge\\ 1 of PCL-5 items 2, 3
...
A minimum of 3 symptoms total across all ICD-11 items (2, 3, 6, 7, 17,
18) must be present. This is automatically satisfied when all three
...
re-experiencing to have a here-and-now quality, which nightmares (item
2) and flashbacks (item 3) capture, while intrusive memories (item 1) as
worded in the PCL-5 do not. To benchmark the broader seven-item variant
(items 1, 2, 3, 6, 7, 17, 18) instead, compute it yourself and pass it
to
[`compare_optimizations`](https://tobiasrspiller.github.io/PTSDdiag/reference/compare_optimizations.md)
as a custom fixed criterion:
